...
{
    public class Quando_ativar_com_situacao_invalida
    {
        ...
        public Quando_ativar_com_situacao_invalida()
        {
        ...
        {
            var action = new Action(() => _service.AtivarAsync(_id).GetAwaiter().GetResult());

            action.Should().Throw<LivroSituacaoInvalidaParaAtivarException>();
        }
    }
}
